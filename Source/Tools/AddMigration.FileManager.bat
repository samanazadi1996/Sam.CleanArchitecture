...
echo dotnet ef migrations add !migrationName! --context FileManagerDbContext --project ..\Src\Infrastructure\!slnName!.Infrastructure.FileManager --startup-project ..\Src\Presentation\!slnName!.WebApi

:: Check if the specified paths actually exist before running the dotnet command
if not exist "..\Src\Infrastructure\!slnName!.Infrastructure.FileManager" (
    echo Error: Path '..\Src\Infrastructure\!slnName!.Infrastructure.FileManager' does not exist.
    exit /b 1
)
...
dotnet ef migrations add !migrationName! --context FileManagerDbContext --project ..\Src\Infrastructure\!slnName!.Infrastructure.FileManager --startup-project ..\Src\Presentation\!slnName!.WebApi
