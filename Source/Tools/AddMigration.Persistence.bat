...
echo dotnet ef migrations add !migrationName! --context Applicationdbcontext --project ..\Src\Infrastructure\!slnName!.Infrastructure.Persistence --startup-project ..\Src\Presentation\!slnName!.WebApi

:: Check if the specified paths actually exist before running the dotnet command
if not exist "..\Src\Infrastructure\!slnName!.Infrastructure.Persistence" (
    echo Error: Path '..\Src\Infrastructure\!slnName!.Infrastructure.Persistence' does not exist.
    exit /b 1
)
...
dotnet ef migrations add !migrationName! --context Applicationdbcontext --project ..\Src\Infrastructure\!slnName!.Infrastructure.Persistence --startup-project ..\Src\Presentation\!slnName!.WebApi
